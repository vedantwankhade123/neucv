...
import { ArrowRight, Eye, Mail, FileText, Loader2, MessageCircle } from 'lucide-react';
...
const Home = () => {
  ...
  const [previewingCoverLetterTemplateId, setPreviewingCoverLetterTemplateId] = useState<string | null>(null);
  ...
  const handleSelectCoverLetterTemplate = async (templateId: string) => {
    if (!user) {
      navigate('/login');
      return;
    }
    ...
    const { v4: uuidv4 } = await import('uuid');
    const { saveCoverLetterToFirestore } = await import('@/lib/firestore-cover-letter-service');
  ...
  return (
    <div className="flex flex-col min-h-screen">
      <header className="bg-transparent border-b p-4 hidden md:flex items-center justify-between flex-shrink-0 no-print h-16">
      ...
      <main className="flex-grow p-4 md:p-8 overflow-y-auto bg-muted/40">
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            >
              ...
              <div className="text-center">
                <h3 className="font-semibold text-lg mb-1">Create Cover Letter</h3>
                <p className="text-sm text-muted-foreground">Write a professional cover letter</p>
              </div>
            </div>
        ...
        <section className="space-y-12">
          ...
          <div>
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-2xl font-bold tracking-tight">Cover Letter Templates</h2>
                <p className="text-muted-foreground mt-1">Matching designs to complete your application</p>
              </div>
              <Button variant="ghost" onClick={() => navigate('/cover-letter-templates')}>
                View All Cover Letters <ArrowRight className="ml-2 h-4 w-4" />
              </Button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
              {Object.entries(coverLetterTemplates).slice(0, 3).map(([id, template], index) => {
                ...
                return (
                  <div key={`cl-${id}`} className="flex flex-col items-center group cursor-pointer" onClick={() => setPreviewingCoverLetterTemplateId(id)}>
                    <div className={cn(
      ...
      <CoverLetterPreviewModal
        isOpen={!!previewingCoverLetterTemplateId}
        onClose={() => setPreviewingCoverLetterTemplateId(null)}
        templateId={previewingCoverLetterTemplateId}
        onSelectTemplate={handleSelectCoverLetterTemplate}
      />
    </div>
  );
};
